...
def _edge(
    ...
    *,
    ...
    return BuiltRoadEdge(
        ...
        length_m=1.0,
        river_crossings=0,
        width_m=11.0 if road_class == "major_local" else 6.0,
        render_order=1 if road_class == "major_local" else 2,
        path_points=pts,
...
def test_culdesac_flags_survive_dedupe_route_and_syntax():
    ...
    edges = [
        _edge("c0", "a0", "a1", "major_local", [Vec2(0.0, 0.0), Vec2(10.0, 0.0)]),
        _edge("c1-cul", "a1", "a2", "major_local", [Vec2(10.0, 0.0), Vec2(20.0, 0.0)], flags=frozenset({"culdesac"})),
        _edge("c2", "a2", "a3", "major_local", [Vec2(20.0, 0.0), Vec2(30.0, 0.0)]),
        _edge(
            "l-cul",
            "l0",
            "l1",
        ...
        _edge("l-dup", "l0b", "l1b", "minor_local", [Vec2(10.0, 5.0), Vec2(14.0, 8.0)]),
    ]

    nodes2, edges2, _ = _dedupe_and_snap(nodes, edges, snap_tol=0.5)
    local_edges = [e for e in edges2 if e.road_class == "minor_local"]
    ...
    local_after_route = [e for e in edges2 if e.road_class == "minor_local"][0]
    ...
    assert any("local_rerouted" in set(getattr(e, "flags", [])) for e in flagged_edges if str(getattr(e, "road_class", "")) == "minor_local")
