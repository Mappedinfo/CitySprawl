...
def _styled_axis_angle_deg(angle_deg: float, road_style: str, rng: np.random.Generator, kind: str) -> float:
    ...
    if style == "organic":
        jitter = 16.0 if kind == "major_local" else 22.0
        return _normalize_angle_deg(a + float(rng.uniform(-jitter, jitter)))
    if style == "skeleton":
        return a
    # mixed_organic
    jitter = 6.0 if kind == "major_local" else 10.0
    return _normalize_angle_deg(a + float(rng.uniform(-jitter, jitter)))
...
def _nearest_road_tangent_angle_deg(poly, nodes: Sequence[BuiltRoadNode], edges: Sequence[BuiltRoadEdge]) -> Optional[float]:
    ...
    best_angle = None
    for edge in edges:
        if edge.road_class not in ("arterial", "major_local"):
            continue
...
def _freeze_major_network(
    *,
    ...
    for edge in edges:
        if str(getattr(edge, "road_class", "")).lower() in {"arterial", "major_local"}:
...
def _compute_local_coverage_stats(
    *,
    ...
    buffers = []
    for edge in edges:
        if str(getattr(edge, "road_class", "")) not in {"arterial", "major_local", "minor_local"}:
            continue
...
def _generate_hierarchy_linework(
    *,
        ...
        if collector_backend != "classic_turtle":
            for bi, block in enumerate(collector_blocks):
                ...
                for part in _iter_polys_geom(geom):
                    ...
                    angle = _styled_axis_angle_deg(angle, style, rng, "major_local")
    ...
    def _new_local_continuity_id(prefix: str = "minor_local") -> str:
        nonlocal local_continuity_seq
        local_continuity_seq += 1
        return f"{prefix}-cont-{int(local_continuity_seq)}"

    def _continuity_from_meta(meta: object, *, default_prefix: str = "minor_local") -> tuple[str, Optional[str]]:
            ...
            try:
                ...
                if not local_traces:
                    ...
                    for trace_idx, pts in enumerate(local_traces):
                        ...
                        continuity_id, parent_continuity_id = _continuity_from_meta(meta, default_prefix="minor_local")
    ...
    if local_backend != "classic_sprawl" or local_need_grid_supplement:
        ...
        for bi, block in enumerate(supplement_source_polys):
            ...
            for part in _iter_polys_geom(geom):
                ...
                if (bi % 2) == 1:
                    angle += 90.0
                angle = _styled_axis_angle_deg(angle, style, rng, "minor_local")
                ...
                for line in lines:
                    ...
                    mid = pts[len(pts) // 2]
                    d_col = min(
                        _nearest_distance_to_road_classes(pts[0], nodes, edges, {"major_local", "arterial"}),
                        _nearest_distance_to_road_classes(pts[-1], nodes, edges, {"major_local", "arterial"}),
                        _nearest_distance_to_road_classes(mid, nodes, edges, {"major_local", "arterial"}),
                    )
                    pending_local_entries.append(
                        {
                            "pts": pts,
...
def _recompute_local_continuity_orders(
    ...
    for idx, edge in enumerate(edges):
        if str(getattr(edge, "road_class", "")) != "minor_local":
            try:
...
def generate_roads(
    ...
    _emit_stream_event(stream_cb, {"event_type": "road_phase_start", "data": {"phase": "major_local"}})
    ...
    _emit_stream_event(stream_cb, {"event_type": "road_phase_complete", "data": {"phase": "major_local"}})
    
    # --- Phase 3: Local generation ---
    _emit_stream_event(stream_cb, {"event_type": "road_phase_start", "data": {"phase": "minor_local"}})
    ...
    _emit_stream_event(stream_cb, {"event_type": "road_phase_complete", "data": {"phase": "minor_local"}})
    ...
    try:
        ...
        edges, syntax_notes_result, syntax_numeric_result = apply_width_guidance_postprocess(
            ...
            target_classes={"arterial", "major_local"},
        )
    ...
    local_cul_final = sum(1 for e in edges if str(getattr(e, "road_class", "")) == "minor_local" and _has_edge_flag(e, "culdesac"))
    local_edges_final = [e for e in edges if str(getattr(e, "road_class", "")) == "minor_local"]
